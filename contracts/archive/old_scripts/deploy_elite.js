...
async function main() {
    console.log("🚀 Desplegando Tier Elite (50→100 USDT)...\n");
    ...
    console.log("⏳ Desplegando SanDigital_Elite...");
    const Contract = await hre.ethers.getContractFactory("SanDigital_Elite");
    const contract = await Contract.deploy(USDT_ADDRESS);
    await contract.waitForDeployment();

    const address = await contract.getAddress();

    console.log("\n✅ Tier Elite desplegado en:", address);
    console.log("🔍 Verifica en:", `https://sepolia.etherscan.io/address/${address}`);
    console.log("\n📋 Guarda esta dirección:");
    console.log(`"elite": "${address}",`);
    console.log("\n" + "=".repeat(60));
}
