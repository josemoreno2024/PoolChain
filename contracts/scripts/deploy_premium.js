...
async function main() {
    console.log("🚀 Desplegando Tier Premium (40→80 USDT)...\n");
    ...
    console.log("⏳ Desplegando SanDigital_Premium...");
    const Contract = await hre.ethers.getContractFactory("SanDigital_Premium");
    const contract = await Contract.deploy(USDT_ADDRESS);
    await contract.waitForDeployment();

    const address = await contract.getAddress();

    console.log("\n✅ Tier Premium desplegado en:", address);
    console.log("🔍 Verifica en:", `https://sepolia.etherscan.io/address/${address}`);
    console.log("\n📋 Guarda esta dirección:");
    console.log(`"premium": "${address}",`);
    console.log("\n" + "=".repeat(60));
}
