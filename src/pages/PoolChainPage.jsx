...
import { usePoolChain } from '../web3/hooks/usePoolChain';
...
function TierDashboard({ tier, onBack, address }) {
    ...
    const handleApprove = async () => {
        try {
            ...
            setSuccessMessage('✅ USDT aprobado exitosamente');
            setShowSuccess(true);
    ...
    const handleBuyTicket = async () => {
        try {
            await buyTicket();
            setSuccessMessage('✅ Ticket comprado exitosamente');
            setShowSuccess(true);
            setTimeout(() => setShowSuccess(false), 3000);
        } catch (error) {
            setErrorMessage('❌ Error al comprar ticket: ' + error.message);
            setShowError(true);
            setTimeout(() => setShowError(false), 5000);
        }
    };
    ...
    const handleExecuteDraw = async () => {
        ...
        try {
            await executeDraw();
            setSuccessMessage('✅ Sorteo ejecutado exitosamente');
            setShowSuccess(true);
            setTimeout(() => setShowSuccess(false), 3000);
        } catch (error) {
    ...
    const handleClaim = async () => {
        try {
            await claimPrize();
            setSuccessMessage('✅ Premio reclamado exitosamente');
            setShowSuccess(true);
            setTimeout(() => setShowSuccess(false), 3000);
        } catch (error) {
            ...
            <div className="dashboard-container">
                ...
                <div className="dashboard-header">
                    <div className="breadcrumb">
                        Dashboard → {tier.name} → Ronda #{currentRound}
                    </div>
                ...
                <div className="actions-card">
                    ...
                    {address && isApproved && !hasParticipated && !poolFilled && (
                        <button
                            ...
                            disabled={isLoading}
                        >
                            {isLoading ? '⏳ Comprando...' : `2. Comprar Ticket (${tier.entry} USDT)`}
                        </button>
                    )}

                    {hasParticipated && !winnersSelected && (
                        <div className="waiting-status">
                            ⏳ Esperando a que se llene el pool...
                        </div>
                    )}
